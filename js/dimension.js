...
const SecretEvent = (() => {
  ...
  const LINES = [
    ...
    { speaker: 'Narrator', portrait: '🦖', text: '65 juta tahun lalu. Detik-detik sebelum meteor menghantam Bumi.' },
  ...
  function _animateMeteor(cvs) {
    const ctx = cvs.getContext('2d');
    const W = cvs.width, H = cvs.height;
    const meteor = { x: W * 0.15, y: -80, vx: W * 0.010, vy: H * 0.012 };
    const trail = [];
    ...
    function frame() {
      ...
      // Update meteor (approach ground, but never impact)
      meteor.x += meteor.vx;
      meteor.y += meteor.vy;
