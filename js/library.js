...
const Settings = {
  open() {
    ...
    // ✅ Sync epilepsy flash toggle state
    ...
    // ✅ Sync FPS toggle state
    const fpsToggle = document.getElementById('togFPS');
    if (fpsToggle) {
      // Sync state (OFF by default)
    }
  },
  close() {
  ...
  // ✅ Method baru untuk toggle looping
  toggleLooping(el) {
    el.classList.toggle('on');
    const enabled = el.classList.contains('on');
    AudioManager.setBGMusicLooping(enabled);
  },
  // ✅ Method baru untuk toggle flash epilepsy
...
const ModeSelect = {
  ...
  startGame(mode) {
    GS.gameMode = mode; // 'normal' atau 'challenge'
    
    if (mode === 'challenge') {
      GS.challengeTime = 600 * 60; // 10 menit real time (600 detik × 60 FPS = 36000 frame)
      GS.enemyMultiplier = 1.5; // 50% lebih banyak enemy
    } else {
      GS.challengeTime = 0;
