...
const World = (() => {
  ...
  function init() {
    ...
    // Dimension portals
    objects.push({ type:'dimension', x:380,  y:180,  r:32, dimId:1, label:'Dimensi Paleolitikum', visited:false });
    objects.push({ type:'dimension', x:700,  y:700,  r:32, dimId:2, label:'Dimensi Neolitikum',   visited:false });
    objects.push({ type:'dimension', x:1100, y:220,  r:32, dimId:3, label:'Dimensi Mesozoikum',   visited:false });
    objects.push({ type:'dimension', x:1400, y:1200, r:32, dimId:4, label:'Dimensi Perundagian',  visited:false });
  ...
  function draw(ctx) {
    ...
    // Time of day background colors - more vibrant
    let bgColor = '#141e0e';
    let tintAlpha = 0.07;
    let overlayColor = null;
    let overlayAlpha = 0;
    let lightingBoost = 0; // For torch lighting
    ...
    if (GS.timeOfDay === 'day') {
      bgColor = '#3a4a2a'; // Lebih cerah - pagi yang terang
      tintAlpha = 0.05;
    } else if (GS.timeOfDay === 'sore') {
      bgColor = '#4a3a1a'; // Oranye-kecoklatan - sore
      overlayColor = 'rgba(255, 140, 0, 0.25)';
      overlayAlpha = 0.25;
      tintAlpha = 0.08;
    } else if (GS.timeOfDay === 'night') {
      bgColor = '#0a0a14'; // Sangat gelap - malam
      overlayColor = 'rgba(30, 40, 80, 0.45)';
      overlayAlpha = 0.45 - lightingBoost; // Torch reduces darkness
      tintAlpha = 0.15;
    } else if (GS.timeOfDay === 'dawn') {
      bgColor = '#1a0f1a'; // Ungu gelap - fajar
    ...
    // Torch lighting effect (adds yellow glow at night)
    ...
    objects.forEach(obj => {
      ...
      if (obj.type === 'tree') {
        ...
        if (Utils.dist(Player.x, Player.y, obj.x, obj.y) < 100) {
          ...
          ctx.fillText(hintText, sx, sy-57); ctx.textAlign = 'left';
        }
        ...
        // Secret portal dengan lock status - update setiap frame
        ...
        if (Utils.dist(Player.x, Player.y, obj.x, obj.y) < 100) {
          ...
          ctx.fillStyle = '#fff'; ctx.font = '11px sans-serif'; ctx.textAlign = 'center';
          const hintText = obj.locked ? '[E] Butuh Kunci Rahasia' : '[E] MASUK KE BOSS ARENA';
          ctx.fillText(hintText, sx, sy-57); ctx.textAlign = 'left';
        }
        ctx.restore();
      }
    ...
    particles.forEach(p => {
      ...
      // ⚡ Reduce flash for epilepsy safety
      if (GS.flashEpilepsy) {
        alpha = alpha * 0.5; // Half opacity
        // Dampen bright colors (convert to greyscale-ish)
        // This reduces the intensity of bright flashes
        col = '#888888'; // Neutral grey instead of bright colors
      }
  ...
  function _drawPlayer(ctx, px, py) {
    ...
    // ========== EQUIPPED WEAPON HOLDING / SWING ANIMATION ==========
    if (Player.equipped) {
      const weapon = Player.WEAPONS[Player.equipped];
      if (weapon) {
        let weaponX = rightArmX;
        let weaponY = rightArmY;
        let weaponAngle = Player.facing;
        let weaponScale = 1;
        
        // Swing animation when attacking
        if (Player.atkAnim > 0) {
          const atkProgress = 1 - (Player.atkAnim / weapon.atkAnim);
          // Swing motion - rotate weapon around player center during attack
          weaponAngle = Player.facing + Math.sin(atkProgress * Math.PI) * 1.2;
          // Move weapon forward during swing
          ...
          weaponScale = 1 + Math.sin(atkProgress * Math.PI) * 0.2; // Grow during swing
        } else {
          // Normal idle position - weapon in right hand
