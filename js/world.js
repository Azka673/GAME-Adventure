...
const World = (() => {
  ...
  function spawnReturn(x, y) {
    // Partikel jatuh dari atas ke posisi player
    for (let i = 0; i < 20; i++) {
      particles.push({ x: x + (Math.random() - 0.5) * 200, y: -50, vx: (Math.random() - 0.5) * 2, vy: 2 + Math.random() * 3, life: 100, col: '#00ffcc', sz: 4 });
    }
  }
  ...
  function draw(ctx) {
    ...
    // Camera clamp inside map
    cam.x = Utils.clamp(cam.x, W/2, MAP_SIZE - W/2);
    cam.y = Utils.clamp(cam.y, H/2, MAP_SIZE - H/2);
    ...
    for (let x = 0; x < MAP_SIZE; x += 40) { ctx.beginPath(); ctx.moveTo(x+ox,oy);     ctx.lineTo(x+ox,MAP_SIZE+oy); ctx.stroke(); }
    for (let y = 0; y < MAP_SIZE; y += 40) { ctx.beginPath(); ctx.moveTo(ox,y+oy);     ctx.lineTo(MAP_SIZE+ox,y+oy); ctx.stroke(); }
    ctx.restore();
  ...
  function _drawPlayer(ctx, px, py) {
    ...
    // Equipped item in hand (simple 2D overlay)
    if (Player.equipped) {
      const hx = px + Math.cos(Player.facing) * 18;
      const hy = py + Math.sin(Player.facing) * 18;
      ctx.font = '18px serif';
      ctx.fillText(Player.equipped, hx - 8, hy + 6);
    }
    ctx.restore();
  }
