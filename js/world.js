...
const World = (() => {
  ...
  const MM = { x:0, y:0, size:220, scale:0 }; // will be computed in draw
  ...
  function _drawMinimap(ctx, W, H) {
    ...
    const mmSize = MM.size;
    const mmX    = W - mmSize - 16;
    const mmY    = H - mmSize - 96; // above bottom bar
    ...
    ctx.fillText('🗺 MINIMAP  [M]', mmX + mmSize/2, mmY - 4);
    ctx.textAlign = 'left';
    ...
    objects.forEach(o => {
      ...
      ctx.beginPath();
      ctx.arc(mmX + o.x*scale, mmY + o.y*scale, 2, 0, Math.PI*2);
      ctx.fill();
    });
    ...
    objects.forEach(o => {
      ...
      ctx.beginPath();
      ctx.arc(mmX + o.x*scale, mmY + o.y*scale, 4, 0, Math.PI*2);
      ctx.fill();
      ctx.globalAlpha = 1;
    });
    ...
    ctx.fillStyle = '#cc88ff';
    ctx.beginPath();
    ctx.arc(mmX + 2200*scale, mmY + 2200*scale, 4, 0, Math.PI*2);
    ctx.fill();

    // Structures highlights (campfires = orange)
    STRUCTURES.forEach(s => {
      if (s.type === 'campfire') {
        ctx.fillStyle = '#ff8844';
        ctx.beginPath();
        ctx.arc(mmX + s.x*scale, mmY + s.y*scale, 2, 0, Math.PI*2);
        ctx.fill();
      }
    });
    ...
    if (blink) {
      ...
      ctx.lineTo(
        mmX + Player.x*scale + Math.cos(Player.facing)*8,
        mmY + Player.y*scale + Math.sin(Player.facing)*8
      );
      ctx.stroke();
    }
    ...
    ctx.fillStyle = '#000a';
    ctx.beginPath();
    ctx.roundRect(mmX, mmY + mmSize + 2, mmSize, 38, 4);
    ctx.fill();
    ctx.globalAlpha = 1;
    ctx.font = '9px sans-serif'; ctx.textAlign = 'left';
    const legend = [
      { col:'#fff',    label:'● Kamu'     },
      { col:'#2a8a2a', label:'● Pohon'    },
      { col:'#ffcc00', label:'● Chest'    },
      { col:'#cc88ff', label:'● Portal'   },
      { col:'#ff8844', label:'● Api'      },
    ];
    legend.forEach((l, i) => {
      ctx.fillStyle = l.col;
      ctx.fillText(l.label, mmX + 4 + (i % 3) * 72, mmY + mmSize + 14 + Math.floor(i/3)*14);
    });
    ctx.restore();
  }
