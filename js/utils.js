...
const Learning = (() => {
  ...
  function show(topic = 'general') {
    ...
    // Display weapon asset if available
    const iconDiv = document.getElementById('learnIcon');
    if (iconDiv) {
      if (item.img) {
        iconDiv.innerHTML = `<img src="${item.img}" alt="${item.title}">`;
      } else {
        iconDiv.innerHTML = '';
      }
    }
    
    // Display engagement prompt if available
    const engagementDiv = document.getElementById('learnEngagement');
    if (engagementDiv) {
      if (item.engagement) {
        engagementDiv.innerHTML = item.engagement;
      } else {
