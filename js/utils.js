...
const Learning = (() => {
  ...
  // Disimpan di localStorage agar PERSISTEN walau refresh / reload
  const STORAGE_KEY = 'praaksara_shown_facts';
  ...
  // Ambil fakta yang BELUM PERNAH ditampilkan dari topik tertentu
  function _getUnseenFact(topic) {
    const list   = FACTS[topic] || FACTS.general;
    const shown  = _loadShown();

    // Kumpulkan indeks yang belum muncul
    const unseen = [];
    list.forEach((_, i) => {
      if (!shown.has(`${topic}:${i}`)) unseen.push(i);
    });

    // Semua sudah muncul → tidak tampilkan apapun
    if (unseen.length === 0) return null;

    // Pilih acak dari yang belum muncul, tandai sebagai sudah dilihat
    const pick = unseen[Math.floor(Math.random() * unseen.length)];
    shown.add(`${topic}:${pick}`);
    _saveShown(shown);
    return list[pick];
  }
  ...
  // Reset semua tracking — panggil ini saat New Game / restart
  function reset() {
