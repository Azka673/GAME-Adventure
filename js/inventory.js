...
const Inventory = (() => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (it) {
        ...
        if (it.e in FOOD_ITEMS) {
          ...
          // Torch - toggle lighting (nyalakan di inventory)
          slot.classList.add('torch');
          slot.onclick = () => { Utils.notify('🔦 Obor memberikan cahaya di malam hari!', '#ffcc00'); _buildUI(); };
  ...
  function _updateHotbar() {
    ...
    order.forEach((e, i) => {
      ...
      // Add click handler untuk equip item dari hotbar
      const slot = document.getElementById('hs' + i);
      if (slot) {
        slot.onclick = () => {
          if (it) {
            if (Player.WEAPONS && Player.WEAPONS[it.e]) {
              Player.equip(it.e);
            } else if (Player.ARMORS && Player.ARMORS[it.e]) {
              Player.equipArmor(it.e);
            } else if (Player.SHIELDS && Player.SHIELDS[it.e]) {
              Player.equipShield(it.e);
            }
          }
        };
      }
