...
const Player = {
  ...
  armor: null,    // emoji of equipped armor
  shield: null,   // emoji of equipped shield (durability-based)
  shieldDurability: 0, // durability of current shield
  projectiles: [],
  skillCooldown: 0, // weapon skill cooldown
  skillActive: false, // apakah skill sedang aktif
  stuckArrows: 0,  // number of arrows stuck in player
  ...
  // ---- Weapon definitions with SKILLS ----
  WEAPONS: {
    '⚔': { 
  ...
  // ---- Armor definitions ----
  ARMORS: {
    '🛡': { name: 'Perisai Kayu', def: 0.85, desc: 'Mengurangi 15% damage' },
    '🧥': { name: 'Baju Kulit', def: 0.80, desc: 'Mengurangi 20% damage' },
    '🪨': { name: 'Mantel Batu', def: 0.75, desc: 'Mengurangi 25% damage' },
    '🔔': { name: 'Baju Besi', def: 0.70, desc: 'Mengurangi 30% damage - ARMOR TERBAIK!' },
  },

  // ---- Shield definitions with durability ----
  SHIELDS: {
  ...
  equipArmor(emoji) {
    ...
    this.armor = (this.armor === emoji) ? null : emoji;
    Utils.notify(this.armor ? `🛡 Armor: ${emoji} ${this.ARMORS[emoji].desc}` : '🛡 Remove Armor', '#ffaa00');
    return true;
  },
  ...
  // Attempt to block damage with shield
  attemptShieldBlock(damage) {
    ...
    // Block successful - reduce durability
  ...
  _skillSweep(weapon) {
    // ⚡ SWEEP: Attack in wide radius
    Utils.notify(`⚡ SWEEP ATTACK!`, '#ffff00');
    World.spawnBurst(this.x, this.y, '#ffff00');
  ...
  _skillPiercing(weapon) {
    // 💫 PIERCING SHOT: Tembus multiple enemies
  ...
  update(jDx, jDy, keys) {
    ...
    if (GS.returnAnim) {
      ...
      if (this.y >= 200) {
        GS.returnAnim = false;
        this.y = 200;
        World.spawnBurst(this.x, this.y, '#00ffcc');
      }
    }
    ...
    // Bounds
    this.x = Utils.clamp(this.x, 15, 1585);
    this.y = Utils.clamp(this.y, 15, 1585);

    // Hunger / HP drain
    // 1% per detik (≈60 tick/detik)
  ...
  updateProjectiles() {
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      p.y += p.vy;
      p.life--;
      if (p.life <= 0 || p.x < 0 || p.y < 0 || p.x > 1600 || p.y > 1600) {
  ...
  // E = interaksi world (portal/peti/rahasia) kalau dekat, kalau tidak → quiz
  interact() {
    if (Dialog.isOpen()) { Dialog.next(); return; }

    // Secret Portal with Rahasia Key
    ...
    for (const obj of World.objects) {
      if (obj.type === 'chest' && !obj.opened && Utils.dist(obj.x, obj.y, Player.x, Player.y) < 55) {
        obj.opened = true;
        // Check for secret key spawn (20% chance)
    ...
    // Secret zone — hanya terlihat, tidak bisa dimasuki
    for (const obj of World.objects) {
    ...
    // Kalau tidak dekat objek interaksi, E tidak memunculkan soal (soal via tombol ❓)
