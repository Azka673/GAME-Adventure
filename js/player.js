...
const Player = {
  ...
  // ---- Armor definitions with HP bonuses ----
  ARMORS: {
  ...
  equipArmor(emoji) {
    ...
    if (this.armor === emoji) {
      // Remove armor - restore HP to 100
      this.maxHp = 100;
      this.hp = Math.min(this.hp, this.maxHp);
      this.armor = null;
      Utils.notify('🛡 Remove Armor', '#ffaa00');
    } else {
      // Equip armor - set HP based on armor bonus
  ...
  _skillSweep(weapon) {
    ...
    for (const animal of AnimalSystem.animals) {
      ...
      if (Utils.dist(this.x, this.y, animal.x, animal.y) < weapon.skillRadius) {        // 🎯 Alert the animal when hit (if not a mob)
        if (!animal.nightEnemy) {
          animal.alerted = true;
          animal.alertTimer = 120;
        }        // 🎯 Alert the animal when hit (if not a mob)
        if (!animal.nightEnemy) {
          animal.alerted = true;
          animal.alertTimer = 120;
        }
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      for (const a of AnimalSystem.animals) {
        ...
        if (Utils.dist(p.x, p.y, a.x, a.y) < (a.r || 18)) {
          ...
          a.alerted = true; a.alertTimer = 160; // Always alert when hit
