...
const AnimalSystem = (() => {
  ...
  function update() {
    animals.forEach(a => {
      ...
      // ✅ WEATHER EFFECTS
      const weatherMult = WeatherSystem.getEnemyAggrMult();
      const currentWeather = WeatherSystem.getCurrentWeather();
      
      // Darkness = auto alert nearby enemies
      if (currentWeather === 'darkness' && pd < 300) {
        a.alerted = true;
        a.alertTimer = 120;
      }
      ...
      if (a.e === '🐗' && a.aggressive && a.alerted && pd < 200 && pd > 50 && a.atkTimer <= 0 && !a.stunned) {
        ...
        const chargeDamage = Math.max(1, Math.ceil(Player.hp * (a.atkPct * 1.5))); // 50% more damage
      ...
      if (a.ranged && a.aggressive && a.alerted && pd < 400 && a.atkTimer <= 0 && !a.stunned) {
        // Skeleton shoots with some inaccuracy (affected by weather)
        const weatherInaccuracy = currentWeather === 'rain' ? 150 : 100;
        ...
        a.atkTimer = (80 + Math.random() * 40) / weatherMult; // Faster in bad weather
      }
      
      // Melee attack with weather multiplier
      if (a.aggressive && a.alerted && pd < 42 && a.atkTimer <= 0 && !a.stunned && !a.ranged) {
        const damage = Math.max(1, Math.ceil(Player.hp * a.atkPct * weatherMult));
