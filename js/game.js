...
const GameCore = (() => {
  ...
  function _loop() {
    ...
    // 🎮 DRAW FPS ON CANVAS (if setting enabled)
    ...
    if (timerEl) {
      if (GS.gameMode === 'challenge') {
        // ✅ SHOW CHALLENGE TIMER (convert frames to seconds)
        ...
        timerEl.style.color = GS.challengeTime < 120 * 60 ? '#ff6666' : '#ffaa00'; // Red at < 2 min real time
      } else {
    ...
    // ✅ SHOW WEATHER STATUS IN HUD
    const zoneEl = document.getElementById('hZone');
    if (zoneEl && GS.gameMode === 'challenge') {
      zoneEl.textContent = `Challenge Mode`;
    }
  }
