...
const MissionSystem = (() => {
  const MISSIONS = [
    ...
    { id:'quiz',   label:'Jawab 15 Soal ❓',              done:false },
    { id:'secret', label:'RAHASIA: Kalahkan Dragon Proto 🦖 ⭐', done:false },
  ];
  ...
  function check() {
    ...
    if (GS.quizAnswered >= 15)  MISSIONS[4].done = true;
...
const GameCore = (() => {
  ...
  function _equipFromHotbar(slotIdx) {
    ...
    if (!item) return;
    if (Player.WEAPONS && Player.WEAPONS[item.e])      Player.equip(item.e);
    else if (Player.ARMORS && Player.ARMORS[item.e])   Player.equipArmor(item.e);
    else if (Player.SHIELDS && Player.SHIELDS[item.e]) Player.equipShield(item.e);
  }
  ...
  function _setupInput() {
    ...
    window.addEventListener('keyup', e => {
      ...
      if (e.code === 'KeyM') World.toggleMinimap();
    }, true);
    ...
    canvas.addEventListener('click',    e => { if (!handleTap(e.clientX, e.clientY)) Player.attack(); });
  ...
  function start() {
    ...
    AudioManager.init();
    if (AudioManager.getBGMusicEnabled()) AudioManager.playBGMusic();
  ...
  function _playIntro() {
    ...
    let idx = 0;
    function showLine() {
      if (idx >= lines.length) { Dialog.close(); return; }
      const line = lines[idx];
      Dialog.show(line.speaker, line.portrait, [line.text], null);
      document.getElementById('dialogNext').onclick = () => { idx++; showLine(); };
    }
    showLine();
  }
  ...
  function _loop() {
    ...
    if (GS.paused) { World.draw(ctx); return; }

    fpsFrames++;
    const now = Date.now();
    if (now - fpsTime >= FPS_UPDATE_INTERVAL) {
      fpsDisplay = Math.round((fpsFrames * 1000) / (now - fpsTime)) + ' FPS';
      fpsFrames  = 0;
      fpsTime    = now;
    }

    if (document.getElementById('togFPS')?.classList.contains('on')) {
      ctx.save();
      ctx.font      = 'bold 16px monospace';
      ctx.fillStyle = fpsDisplay.includes('60') ? '#00ff00' : '#ffaa00';
      ctx.fillText(fpsDisplay, 20, 30);
      ctx.restore();
    }
    ...
    if (GS.gameMode === 'challenge' && GS.challengeTime > 0) {
      GS.challengeTime--;
      if (GS.challengeTime <= 0) {
        GS.started = false;
        Utils.notify('⏳ WAKTU HABIS! Challenge selesai!', '#ff6666');
        EndingSystem.showBadEnding();
        return;
      }
    }
    ...
    if (timerEl) {
      if (GS.gameMode === 'challenge') {
        const totalSeconds = Math.floor(GS.challengeTime / 60);
        const mins = Math.floor(totalSeconds / 60);
        const secs = totalSeconds % 60;
        timerEl.textContent = `⏱️ CHALLENGE: ${mins}:${secs.toString().padStart(2, '0')}`;
        timerEl.style.color = GS.challengeTime < 120 * 60 ? '#ff6666' : '#ffaa00';
      } else {
