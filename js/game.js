...
const GameCore = (() => {
  ...
  console.log('[GAME] Core initialized - canvas ready');
  ...
  function _setupInput() {
    ...
    if (!jOut || !jIn) { console.warn('[INPUT] Joystick elements not found'); return; }
    ...
    function joyMove(clientX, clientY) {
      ...
      jDx = dx / JOY_R;
      jDy = dy / JOY_R;
      console.log('[JOYSTICK]', jDx.toFixed(2), jDy.toFixed(2));
    }
    ...
    console.log('[INPUT] All input handlers setup complete - READY!');
  }
  ...
  function _loop() {
    ...
    // Debug keyboard state every 30 frames
    if (GS.tick % 30 === 0) {
      const activeKeys = Object.keys(keys).filter(k => keys[k] === true);
      console.log(`[FRAME ${GS.tick}] Active keys:`, activeKeys.length > 0 ? activeKeys : '(none)');
    }
