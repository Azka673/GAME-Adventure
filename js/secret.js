...
const SecretSystem = (() => {
  ...
  function initSecretKey() {
    // Peluang spawning secret key di chest (20% chance setiap kali chest di-respawn)
    if (Math.random() < 0.20) {
      return { 
        e: '🔑', 
        n: 'Kunci Rahasia', 
  ...
  function bossDamage(amount) {
    ...
    if (boss.hp <= 0) {
      ...
      if (Player.hp <= 0) {
        ...
        const attacks = [
          `🦖 Dragon mencabik dengan cakarnya! [${damage} DMG]`,
          `🦖 Dragon menggeram menggelegar! [${damage} DMG]`,
          `🦖 Dragon mencengkeram dengan mulutnya! [${damage} DMG]`,
          `🦖 Dragon menyapu ekor raksasanya! [${damage} DMG]`,
        ];
